...
import DynamicFeatherIcon from "@myComponenets/DynamicFeatherIcon";
...
export default function BusinessTap({ selectedUser }) {
  ...
  // ** Queries
  const userProfessionsQuery = useSWR("user_profession", getUserProfessions);
  const carTypesQuery = useSWR("car_types", getCarTypes);
  const carBrandsQuery = useSWR("car_brands", getCarBrands);
  const partCategoriesQuery = useSWR("part_categories", getPartCategories);
  // ** Mutations
                    ...
                    <Col md={4} className="mb-md-0 mb-2" key={item.name}>
                      ...
                      >
                        <span className="d-flex align-items-center mb-50">
                          <DynamicFeatherIcon iconName={item.icon} className="me-1" />
